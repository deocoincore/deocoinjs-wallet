...
This is a client-side wallet library that can generate private keys from a mnemonic, or import private keys from other deocoin wallets.

It can sign transactions locally, and submit the raw transaction data to a remote deocoin node. The blockchain data is provided by the Insight API, rather than the raw deocoind RPC calls.

This library makes it possible to run a DApp without the users having to run a full deocoind node.
...
```js
import { networks, generateMnemonic } from "deocoinjs-wallet"
...
The currency unit used is `satoshi`. To convert deocoin to satoshi you should multiply the amount you want with `1e8`.

```js
import { networks } from "deocoinjs-wallet"
...
The ABI encoding for the `burnbabyburn()` invokation is `e179b912`. We'll burn 0.05 deocoin, expressed in unit of satoshi.
...
async function main() {
  ...
  const tx = await wallet.contractSend(contractAddress, encodedData, {
    amount: 0.05 * 1e8, // 0.05 deocoin in satoshi
  })
...
Suppose you want to import the public address `SRoinb4dd2M63T8qHdPd9iN9e2TkcitgTQ`. Use `deocoin-cli` to dump the private key from wallet:

```bash
./deocoin-cli dumpprivkey "SRoinb4dd2M63T8qHdPd9iN9e2TkcitgTQ"
...
const amount = 0.15 * 1e8 // 0.15 deocoin
...
```ts
const tx = await wallet.send(toAddress, amount, {
  // rate is 400 satoshi per byte, or  ~0.004 deocoin/KB, as is typical.
  feeRate: 400,
})
```
...
You can decode the raw transaction using `deocoin-cli`:

```bash
./deocoin-cli decoderawtransaction 0100000001a09a921ecb38...
...
```bash
./deocoin-cli decoderawtransaction 0100000001e7b6f2767e8a6...
```
